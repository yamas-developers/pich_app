...
class EditVendorProduce extends StatefulWidget {
  static const String routeName = "/edit_vendor_produces_screen";

  EditVendorProduce({Key? key}) : super(key: key);

  @override
  State<EditVendorProduce> createState() => _EditVendorProduceState();
}

class _EditVendorProduceState extends State<EditVendorProduce> {
  int quantity = 1;
  ...
  getPageData() async {
    StoreProduceBag bag = ModalRoute.of(context)?.settings.arguments as StoreProduceBag;
      setState(() {
        storeProduce = bag;
        _price = bag.price.toString();
        bagProducts = bag.produceBagProduct!;
        status = bag.status!;
      });
    txtBagName.text = storeProduce!.produceBagTitle!;
  ...
  submit() async {
    ...
    var payload = {
      ...
      "produce_bag_title": txtBagName.text,
      "price": _price,
      "status":status,
    ...
    var response = await MjApiService()
        .postRequest("${MJ_Apis.edit_produces}/${storeProduce!.id}", payload);
    hideProgressDialog(context);
    if (response != null) {
      print("responseOfEditProduce: ${response}");
...
Container ProducesListContainer({
  ...
  return Container(
    margin: EdgeInsets.fromLTRB(10, 1, 10, 7),
    height: 80,
    decoration: BoxDecoration(
      color: itmGreyColor,
...
//                   fontSize: 14,
//                   onPressed: () {
//                     Navigator.pushNamed(context, VendorProduceBagsScreen.routeName);
//                   },
//                 ),
//               ),
//             ),
