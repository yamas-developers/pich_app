...
class ChatFollowingsArea extends StatefulWidget {
  const ChatFollowingsArea({
    Key? key,
    required this.type,
  }) : super(key: key);
  final String type;
  ...
  @override
  State<ChatFollowingsArea> createState() => _ChatFollowingsAreaState();
}

class _ChatFollowingsAreaState extends State<ChatFollowingsArea> {
  ...
  getPageData() async {
    ...
    // if (context.read<FollowingsProvider>().currentPage < 1)
    getConectionsData();
    setState(() {
      isLoading = false;
    });
  ...
  reset() async {
    context.read<FollowingsProvider>().currentPage = 0;
    context.read<FollowingsProvider>().maxPages = 0;
    context.read<FollowingsProvider>().list = [];
    context.read<FollowingsProvider>().isLoading = false;
  ...
  Future<bool> getConectionsData() async {
    context.read<FollowingsProvider>().isLoading = true;
    String queryString;
    ...
    if (response != null) {
      ...
      context.read<FollowingsProvider>().currentPage = response['current_page'];
      context.read<FollowingsProvider>().maxPages = response['last_page'];
      context.read<FollowingsProvider>().list = list;
    }
    return true;
  }
  ...
  Future<bool> loadMoreData() async {
    ...
    int page = context.read<FollowingsProvider>().currentPage + 1;
    if (page > context.read<FollowingsProvider>().maxPages) {
    ...
    if (response != null) {
      ...
      context.read<FollowingsProvider>().currentPage = response['current_page'];
      context.read<FollowingsProvider>().maxPages = response['last_page'];
      context.read<FollowingsProvider>().addMore(list);
    }
    return true;
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<FollowingsProvider>(builder: (key, provider, child) {
      return Column(
        children: [
          Container(
